...
export default function PostVehicle() {
  ...
  const canSubmit = form.startLocation.trim() && form.destination.trim() && form.travelDate && form.travelTime && form.totalSeats >= 1 && !loading;
  ...
  const inputClass = (key) =>
    `w-full border rounded-md pl-9 pr-3 py-2 focus:outline-none focus:ring-2 transition ${errors[key] ? "border-rose-400 focus:ring-rose-200" : "border-slate-300 dark:border-slate-600 dark:bg-slate-900 focus:ring-accent"}`;

  const submit = async (e) => {
    e.preventDefault();
    setTouched({ startLocation: true, destination: true, travelDate: true, travelTime: true, totalSeats: true });
    if (!canSubmit) return;
    try {
      setLoading(true);
      ...
      />
        ...
        <form onSubmit={submit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 space-y-4" noValidate>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Vehicle type</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Vehicle model</label>
              <div className="relative">
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Vehicle number</label>
            <div className="relative">
              <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input {...field("vehicleNumber")} onChange={(e) => setForm({ ...form, vehicleNumber: e.target.value.toUpperCase() })}
                className={inputClass("vehicleNumber")} placeholder="e.g. TN 59 AB 1234" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">From *</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">To *</label>
              <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Available seats</label>
              <div className="relative">
                <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input type="number" min={1} {...field("totalSeats")}
                  onChange={(e) => setForm({ ...form, totalSeats: Number(e.target.value) })}
                  className={inputClass("totalSeats")} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Fare per seat (₹)</label>
              <div className="relative">
                <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input type="number" min={0} {...field("farePerSeat")}
