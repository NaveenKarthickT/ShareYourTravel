...
const STATUS_COLORS = {
  requested: "#f59e0b", confirmed: "#06b6d4", completed: "#64748b",
  rejected: "#f43f5e", cancelled: "#94a3b8",
};
const ROLE_COLORS = { org_admin: "#0B2B4F", member: "#00A3C4" };
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 px-6 py-8 max-w-6xl">
        ...
        {loading ? <SkeletonStatRow count={4} /> : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <Card><div className="text-3xl font-bold text-primary dark:text-sky-300">{summary?.totalTrips ?? 0}</div><div className="text-xs text-slate-500 mt-1">Total trips</div></Card>
              <Card><div className="text-3xl font-bold text-primary dark:text-sky-300">{summary?.totalBookings ?? 0}</div><div className="text-xs text-slate-500 mt-1">Bookings</div></Card>
              <Card><div className="text-3xl font-bold text-primary dark:text-sky-300">{summary?.completedTrips ?? 0}</div><div className="text-xs text-slate-500 mt-1">Completed</div></Card>
              <Card><div className="text-3xl font-bold text-primary dark:text-sky-300">{summary?.activeUsers ?? 0}</div><div className="text-xs text-slate-500 mt-1">Active members</div></Card>
            </div>
              ...
              <Card title="Trips posted (last 30 days)">
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={tripsPerDay}>
                    ...
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#00A3C4" strokeWidth={2} dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </Card>
              <Card title="Top 5 routes">
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={topRoutes} layout="vertical" margin={{ left: 20 }}>
                    ...
                    <YAxis dataKey="route" type="category" fontSize={10} width={130} />
                    <Tooltip />
            ...
            <div className="grid lg:grid-cols-2 gap-5">
              <Card title="Booking status">
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    <Pie data={statusBreakdown} dataKey="count" nameKey="status" innerRadius={50} outerRadius={90} paddingAngle={2}>
                      {statusBreakdown.map((e) => <Cell key={e.status} fill={STATUS_COLORS[e.status] || "#94a3b8"} />)}
                    </Pie>
              ...
              <Card title="Members by role">
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    <Pie data={roleBreakdown} dataKey="count" nameKey="role" innerRadius={50} outerRadius={90} paddingAngle={2}>
                      {roleBreakdown.map((e) => <Cell key={e.role} fill={ROLE_COLORS[e.role] || "#94a3b8"} />)}
                    </Pie>
                    <Tooltip /><Legend fontSize={11} />
                  </PieChart>
                </ResponsiveContainer>
