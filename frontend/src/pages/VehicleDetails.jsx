      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm">
        ...
        <div className="grid sm:grid-cols-2 gap-4 text-sm text-slate-700 dark:text-slate-300 mb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-900 dark:text-slate-100">{vehicle.vehicleType} {vehicle.vehicleModel && `· ${vehicle.vehicleModel}`}{vehicle.vehicleNumber && ` · ${vehicle.vehicleNumber}`}</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-emerald-600 dark:text-emerald-400">{vehicle.farePerSeat > 0 ? `₹${vehicle.farePerSeat}/seat` : "Free Ride"}</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {vehicle.notes && (
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-6 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5">{vehicle.notes}</p>
        )}
        ...
        {!isOwner && vehicle.status === "available" && (
          <div id="request" className="flex items-end gap-3 scroll-mt-24 p-5 bg-cyan-50/70 dark:bg-cyan-950/30 rounded-2xl border border-cyan-200/80 dark:border-cyan-800/50">
            <div>
              <label className="block text-xs font-semibold mb-1 text-slate-700 dark:text-slate-300">Seats to request</label>
              <input
                type="number"
                min={1}
                ...
                className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 w-24 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#00A3C4]"
              />
            </div>
            <button
              onClick={requestSeat}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#00A3C4] to-[#0284C7] text-white font-bold text-sm shadow-md shadow-[#00A3C4]/25 hover:shadow-lg transition"
            >
              Request seat
            </button>
          </div>
        ...
        {!isOwner && vehicle.status === "ongoing" && (
          <div className="bg-cyan-50 dark:bg-cyan-950/40 text-cyan-800 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800 text-sm px-4 py-3 rounded-xl">
            🚗 Trip in progress — check <strong>Ongoing Trip</strong> in the sidebar to chat with the driver.
          </div>
        )}

        {!isOwner && vehicle.status === "completed" && (
          <div className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm px-4 py-3 rounded-xl">
