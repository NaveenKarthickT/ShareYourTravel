...
export function MembershipRequests() {
  ...
  const review = async (member, decision) => {
    try {
      ...
      showToast(decision === "approved" ? `Approved ${member.user.name}` : `Rejected ${member.user.name}`, decision === "approved" ? "success" : "error");
      load();
    } catch (err) {
    ...
    <div className="flex">
      ...
      <div className="flex-1 px-6 py-8 max-w-4xl">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-primary dark:text-sky-300">Membership requests</h1>
          <select value={filter} onChange={(e) => setFilter(e.target.value)}
            className="border border-slate-300 dark:border-slate-600 dark:bg-slate-900 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent">
        ...
        : members.length === 0 ? (
          ...
          <div className="space-y-2">
            {members.map((m) => (
              <div key={m._id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-slate-800 dark:text-slate-200">{m.user.name}</div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {m.status === "pending" && (
                    <>
                      <button onClick={() => review(m, "approved")} className="text-xs px-3 py-1.5 rounded-md bg-emerald-600 text-white hover:bg-emerald-700">Approve</button>
                      <button onClick={() => setConfirmTarget(m)} className="text-xs px-3 py-1.5 rounded-md bg-rose-600 text-white hover:bg-rose-700">Reject</button>
                    </>
                  )}
                </div>
              </div>
      ...
      <ConfirmModal open={!!confirmTarget} title="Reject this request?"
...
export function AdminVehicles() {
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 px-6 py-8 max-w-5xl">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-primary dark:text-sky-300">Vehicles & trips</h1>
        ...
        {loading ? <div className="text-slate-500">Loading...</div> : (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800 text-left text-slate-500 text-xs uppercase tracking-wide">
                <tr>
              ...
              <tbody>
                {vehicles.map((v) => (
                  <tr key={v._id} className="border-t border-slate-100 dark:border-slate-800">
                    <td className="p-3 text-slate-800 dark:text-slate-200">{v.startLocation} → {v.destination}</td>
                    <td className="p-3 text-slate-600 dark:text-slate-400">{v.postedBy?.name}</td>
                    <td className="p-3 text-slate-600 dark:text-slate-400">{new Date(v.travelDate).toLocaleDateString()}</td>
                    <td className="p-3 text-slate-600 dark:text-slate-400">{v.availableSeats}/{v.totalSeats}</td>
