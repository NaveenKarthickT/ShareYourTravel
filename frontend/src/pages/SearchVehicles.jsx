...
const FILTER_LABELS = {
  startLocation: "From", destination: "To", date: "Date", minSeats: "Min seats",
};
...
export default function SearchVehicles() {
  ...
  const clearAll = () => { setFilters(EMPTY); search(EMPTY); };
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 px-6 py-8 max-w-6xl">
        <h1 className="text-2xl font-bold mb-1 text-primary dark:text-sky-300">Search vehicles</h1>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 mb-4 grid sm:grid-cols-5 gap-3 items-end">
          <div>
            <label className="block text-xs font-medium mb-1 text-slate-600 dark:text-slate-400">From</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1 text-slate-600 dark:text-slate-400">To</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-xs font-medium mb-1 text-slate-600 dark:text-slate-400">Min. seats</label>
            <div className="relative">
          ...
          <button onClick={() => search()} disabled={!canApply}
        ...
        <FilterChips filters={applied} labels={FILTER_LABELS} onRemove={removeFilter} onClearAll={clearAll} />
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 rounded-2xl">
            ...
            <p className="text-slate-500 text-sm mb-6 max-w-sm mx-auto">
              {hasAppliedFilters ? "Try widening your search — different date, fewer seats, or a broader route." : "Start by posting the first trip for your community."}
            </p>
            {hasAppliedFilters ? (
              <button onClick={clearAll} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-md border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">
                Clear filters
              </button>
            ) : (
              <Link to="/vehicles/post" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-md bg-accent text-white text-sm font-medium hover:bg-[#008fad]">
