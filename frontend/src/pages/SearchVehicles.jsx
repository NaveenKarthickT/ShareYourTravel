...
import { MapPin, Flag, Calendar, Users, SearchIcon } from "lucide-react";
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 px-6 py-8 max-w-6xl">
        <h1 className="text-2xl font-bold mb-1">Search vehicles</h1>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6 grid sm:grid-cols-5 gap-3 items-end">
          ...
          <div>
            <label className="block text-xs font-medium mb-1">Min. seats</label>
            <div className="relative">
              <Users className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
              <input type="number" min={1} value={filters.minSeats} onChange={(e) => setFilters({ ...filters, minSeats: e.target.value })}
                className="w-full border border-slate-300 rounded-md pl-8 pr-2 py-1.5 text-sm" />
            </div>
          </div>
          <button onClick={search} className="flex items-center justify-center gap-1.5 bg-brand-600 text-white rounded-md py-2 text-sm font-medium hover:bg-brand-700">
