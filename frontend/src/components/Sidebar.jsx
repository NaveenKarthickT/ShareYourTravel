...
import {
  ...
  Inbox, Flag, Shield, Users, Car, Circle, BarChart3,
} from "lucide-react";
...
const desktopButtonClass = ({ isActive }) =>
  `flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium border transition-colors ${
    isActive
      ...
      : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-accent hover:bg-accent-soft dark:hover:bg-slate-800"
  }`;
...
const mobileButtonClass = ({ isActive }) =>
  `flex flex-col items-center justify-center gap-0.5 shrink-0 w-16 py-1.5 rounded-lg text-[11px] font-medium ${
    isActive ? "bg-accent text-white" : "text-slate-500 dark:text-slate-400"
  }`;

export default function Sidebar({ links }) {
  const { activeOrg } = useAuth();
  const { pendingRequests, totalCount } = useNotifications() || {};
  ...
  const badgeFor = (to) => {
    if (to === "/admin/members" && pending > 0) return pending;
    if (to === "/dashboard" && totalCount > 0) return totalCount;
    if (to === "/admin" && pending > 0) return pending;
    return null;
  };

  return (
    <>
      <aside className="hidden sm:block w-56 shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 min-h-[calc(100vh-4rem)] p-3 space-y-1.5">
        {links.map((l) => {
      ...
      <nav className="sm:hidden fixed bottom-0 inset-x-0 z-40 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 px-2 py-1.5 flex gap-1 overflow-x-auto">
        {allLinks.map((l) => {
          const Icon = iconFor(l.to);
          return (
