...
// One lightweight endpoint the frontend polls for: (1) booking requests
// received on vehicles this user owns that still need a decision, and
// (2) chat threads with unread messages — whether this user is the
// passenger or the vehicle owner on that thread.
export const getSummary = asyncHandler(async (req, res) => {
  const userId = req.user._id;

  const myVehicleIds = (await Vehicle.find({ postedBy: userId }).select("_id")).map((v) => v._id);

  // 1. Pending requests received on my posted vehicles
  const pendingBookings = await Booking.find({
    vehicle: { $in: myVehicleIds },
    status: "requested",
  })
  ...
  // 2. Unread chat messages across every thread I'm part of
  const myBookingsAsPassenger = await Booking.find({
    passenger: userId,
    status: { $in: ["confirmed", "completed"] },
  }).select("_id");
  const myBookingsAsOwner = await Booking.find({
    vehicle: { $in: myVehicleIds },
    status: { $in: ["confirmed", "completed"] },
  }).select("_id");
